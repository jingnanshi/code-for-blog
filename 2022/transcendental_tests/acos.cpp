...
std::vector <std::tuple<double, double>> gen_random_vec(int length) {
  std::vector <std::tuple<double, double>> vec(length);
  for (size_t i = 0; i < vec.size(); ++i) {
    vec[i] = std::make_tuple<double, double>(rand(), rand());
  }
  return vec;
}
...
int main(int argc, char **argv) {
  ...
  std::vector <std::tuple<double, double>> a = gen_random_vec(VECTOR_LENGTH);
  std::vector <std::tuple<double, double>> b = gen_random_vec(VECTOR_LENGTH);
